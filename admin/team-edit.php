...
if(isset($_POST['update'])) {
	...
	if(empty($error)=== true)  {
	    move_uploaded_file($file_tmp,"images/about/team/".$file_name);	
	}  else {
		echo"<pre>";
		echo"print_r($error)";
		echo"</pre>";
       ...
       $sql = "UPDATE team SET IMAGE= '{$file_name}',TITLE = '{$name}',DESCRIPTION= '{$description}' WHERE ID = {$id}";
...
<!--ending of navbar -->
  ...
  <div class="container">
    <h1 style="color:#0c3">Edit Team Card</h1>
  ...
  ?>
     <form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
       ...
       <div class="form-group">
          <label>Name *</label>
          <input type="text" class="form-control" name="NAME" value="<?php echo $row1['TITLE'];?>" required>
       </div>
      
       <div class="form-group">
          <label>Description *</label>
          <input type="text" class="form-control" name="DESCRIPTION" value="<?php echo $row1['DESCRIPTION'];?>" required>
       </div>
  
       <div class="form-group">
          <label>Image *</label>
          <img src="images/about/team/<?php echo $row1['IMAGE'];?>" style="height:30vh">
